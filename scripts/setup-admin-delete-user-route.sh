#!/bin/bash
# Setup script for admin_delete_user API Gateway route
# This script helps create the DELETE /admin/users/{athlete_id} route in API Gateway

set -e

echo "🚀 RabbitMiles Admin Delete User Route Setup"
echo "============================================="
echo ""
...
if [ -z "$LAMBDA_FUNCTION_NAME" ]; then
    echo "📋 Finding admin_delete_user Lambda function..."
    echo ""
    
    # Try common Lambda function names
    for name in "rabbitmiles-admin-delete-user" "prod-admin-delete-user" "admin-delete-user"; do
    ...
    if [ -z "$LAMBDA_FUNCTION_NAME" ]; then
        ...
        read -p "Enter your admin_delete_user Lambda function name: " LAMBDA_FUNCTION_NAME
    fi
fi
...
echo "🔍 Checking if DELETE /admin/users/{athlete_id} route exists..."
ROUTE_EXISTS=$(aws apigatewayv2 get-routes --api-id "$API_GATEWAY_ID" \
    --query 'Items[?RouteKey==`DELETE /admin/users/{athlete_id}`].RouteId' --output text)

if [ -n "$ROUTE_EXISTS" ]; then
    echo "⚠️  Route DELETE /admin/users/{athlete_id} already exists (Route ID: $ROUTE_EXISTS)"
...
echo "🛣️  Creating DELETE /admin/users/{athlete_id} route..."
...
echo "🛣️  Creating OPTIONS /admin/users/{athlete_id} route for CORS..."
OPTIONS_ROUTE_EXISTS=$(aws apigatewayv2 get-routes --api-id "$API_GATEWAY_ID" \
    --query 'Items[?RouteKey==`OPTIONS /admin/users/{athlete_id}`].RouteId' --output text)
...
echo "Route: DELETE /admin/users/{athlete_id}"
...
echo ""
echo "curl -X DELETE $API_ENDPOINT/admin/users/12345 \\"
echo "  -H \"Cookie: rm_session=your-admin-session-cookie\" \\"
echo "  -v"
echo ""
echo "Expected response for non-existent user (404):"
echo "{\"error\": \"user not found\"}"
echo ""
echo "Expected response for successful deletion (200):"
echo "{\"success\": true, \"deleted\": {...}}"
echo ""
...
echo ""
echo "For detailed instructions, see DEPLOYMENT_DELETE_USER.md"
echo ""
