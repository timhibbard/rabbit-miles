...
function Admin() {
  ...
  const handleRefreshActivities = async () => {
    ...
    let result;
    if (selectedUser) {
      result = await fetchUserActivities(selectedUser.athlete_id, 50, 0);
    } else {
      result = await fetchAllActivities(50, 0);
    }
  ...
  const loadMoreActivities = async () => {
    ...
    let result;
    if (selectedUser) {
      result = await fetchUserActivities(selectedUser.athlete_id, 50, activitiesOffset);
    } else {
      result = await fetchAllActivities(50, activitiesOffset);
    }
  ...
  const getActivityCountText = () => {
    if (!selectedUser) {
      const baseText = "All Athletes' Activities";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 flex gap-3">
        <button
          ...
          className="inline-flex items-center px-4 py-2 border border-purple-300 shadow-sm text-sm font-medium rounded-md text-purple-700 bg-white hover:bg-purple-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
          title="Recalculate leaderboard aggregates from all activities"
        >
          {recalculatingLeaderboard ? (
            <>
              <svg className="animate-spin -ml-0.5 mr-2 h-4 w-4 text-purple-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div className="divide-y divide-gray-200 max-h-[600px] overflow-y-auto" onScroll={handleScroll}>
            {activitiesLoading ? (
              ...
              <div className="px-6 py-8 text-center text-gray-500">
                {selectedUser ? 'No activities found for this user' : 'No activities found'}
              </div>
            ) : (
              <>
